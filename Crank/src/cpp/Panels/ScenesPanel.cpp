...
void ScenesPanel::updateCurrentSceneFromActive() {
    ...
    currentScene = g_ActiveProject.getActiveScene().asScene();
...
void ScenesPanel::render() {
    ...
    else { ImGui::Text("No scene selected"); }
...
void ScenesPanel::drawEntityNode(Entity entity) {
    ...
    bool opened = false;
    if (!isEntityBeingEdited[static_cast<uint32_t>(entity)])
        opened = ImGui::TreeNodeEx(
            (void*)static_cast<uint32_t>(entity), flags, tag.c_str());
    else {
        ImGui::InputTextWithHint(("##" + std::to_string(static_cast<uint32_t>(entity))).c_str(),
                                 "Enter a new tag", tag.data(), tag.size() + 1);
        if (ImGui::IsKeyPressed(ImGuiKey_Enter) ||
            (!ImGui::IsItemHovered() && ImGui::IsMouseClicked(ImGuiMouseButton_Left)))
            isEntityBeingEdited[static_cast<uint32_t>(entity)] = false;
    }

    if (!isEntityBeingEdited[static_cast<uint32_t>(entity)] && ImGui::IsItemHovered() &&
        ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left))
        isEntityBeingEdited[static_cast<uint32_t>(entity)] = true;
    else if (!isEntityBeingEdited[static_cast<uint32_t>(entity)] &&
        ImGui::IsItemClicked())
