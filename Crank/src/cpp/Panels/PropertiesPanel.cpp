...
void PropertiesPanel::render() {
    ...
    if (gPanel_ContentBrowser->isItemSelected()) selectedItem = gPanel_ContentBrowser->
        getSelectedItem();
    if (gPanel_SceneHierarchy->getSelectedEntity()) selectedItem = gPanel_SceneHierarchy->
        getSelectedEntity();

    if (!selectedItem.has_value()) { ImGui::Text("Select an entity to view its properties"); }
    ...
    else if (selectedItem.type() == typeid(fs::path)) { materialProperties(); }

    ImGui::PopStyleVar();
}

void PropertiesPanel::entityProperties() {
    auto selectedEntity = std::any_cast<Entity>(selectedItem);
    ...
    if (ImGui::BeginPopup("AddComponent")) {
        if (ImGui::MenuItem("Camera")) {
            ...
            else { AE_CORE_WARN("This entity already has a camera component!"); }
        ...
        if (ImGui::MenuItem("Sprite Renderer")) {
            ...
            else { AE_CORE_WARN("This entity already has a sprite component!"); }
        ...
        if (ImGui::MenuItem("Script")) {
            ...
            else { AE_CORE_WARN("This entity already has a script component!"); }
...
void PropertiesPanel::materialProperties() {
    ...
    if (ImGui::InputText("##Tag", buffer, sizeof(buffer))) { activeMaterial.name = buffer; }

    ImGui::ColorEdit4("Colour", value_ptr(activeMaterial.colour));
    ...
    if (ImGui::BeginTable("##MaterialTable", 2)) {
        ...
        if (auto tex = activeMaterial.getTexture())
            ImGui::ImageButton("##E", (ImTextureID)(*tex)->getSampler().slot,
                               {50.0f, 50.0f}, {0, 1}, {1, 0});
        else ImGui::Button("##Texture", {50.0f, 50.0f});

        if (ImGui::BeginDragDropTarget()) {
            if (const ImGuiPayload* payload =
                ImGui::AcceptDragDropPayload("CONTENTBROWSER_ITEM")) {
                auto dropPayload = static_cast<const DropPayload*>(payload->Data);

                switch (dropPayload->type) {
                    case PayloadType::Texture: activeMaterial.texture = Texture2D::create(
                            g_ActiveProject.getResourcePath(dropPayload->resourceUUID));
                }
    ...
    drawComponent<CameraComponent>(
        ...
        [](auto& component) {
            ...
            if (cam.getProjectionType() == ProjectionType::Perspective) {
                ...
                if (ImGui::DragFloat("Field Of View", &fov, 1.0f, 10.0f, 150.0f)) cam.updateSpec(
                    CameraSpec3D::Feild::FOVorSize, glm::radians(fov));

                if (ImGui::DragFloat("Near", &near, 0.5f, -far, far - 0.01f)) cam.updateSpec(
                    CameraSpec3D::Feild::NearPlane, near);

                if (ImGui::DragFloat("Far", &far, 0.5f, near + 0.01f, 1000.0f)) cam.updateSpec(
                    CameraSpec3D::Feild::FarPlane, far);
            }
            ...
            if (cam.getProjectionType() == ProjectionType::Orthographic) {
                ...
                if (ImGui::DragFloat("Size", &size, 1.0f, 1.0f, 200.0f)) cam.updateSpec(
                    CameraSpec3D::Feild::FOVorSize, size);
