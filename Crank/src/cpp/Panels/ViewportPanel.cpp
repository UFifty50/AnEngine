...
void ViewportPanel::render() {
    ...
    if (ImGui::BeginMenuBar()) {
        if (ImGui::BeginMenu("translation type")) {
            if (ImGui::MenuItem("Position")) gizmoType = ImGuizmo::OPERATION::TRANSLATE;
        ...
        if (ImGui::BeginMenu("Snapping")) {
            ...
            else { ImGui::Text("Snapping not available for this mode."); }
            ImGui::EndMenu();
        }
    ...
    if (ImGui::BeginDragDropTarget()) {
        if (const ImGuiPayload* payload =
            ...
            fs::path resPath = g_ActiveProject.getResourcePath(dropPayload->resourceUUID);

            if (dropPayload->type != PayloadType::Scene) {
                AE_CORE_ERROR("{} is not a scene", resPath.string());
                return;
            }

            if (!FileMenu::OpenScene(resPath)) {
                AE_CORE_ERROR("Couldn't load Scene {}", resPath.string());
                return;
            }
        }
    ...
    auto transform = static_cast<glm::mat4>(eTC);
    ...
    Manipulate(value_ptr(cameraView), value_ptr(cameraProjection),
               static_cast<ImGuizmo::OPERATION>(gizmoType), ImGuizmo::MODE::LOCAL,
               value_ptr(transform), nullptr,
