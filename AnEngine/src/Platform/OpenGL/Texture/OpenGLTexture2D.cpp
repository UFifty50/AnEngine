...
OpenGLTexture2D::OpenGLTexture2D(const fs::path& path)
    : file(path), fileFormat(ImageFormat::FileFormat::NONE) {
    AE_PROFILE_FUNCTION()
    ...
    switch (channels) {
        case 3: this->pixelFormat = ImageFormat::RGB;
            internalFormat = GL_RGB8;
            dataFormat = GL_RGB;
            break;

        case 4: this->pixelFormat = ImageFormat::RGBA;
            internalFormat = GL_RGBA8;
            dataFormat = GL_RGBA;
            break;

        default: this->pixelFormat = ImageFormat::None;
            internalFormat = 0;
            dataFormat = 0;
...
OpenGLTexture2D::OpenGLTexture2D(uint32_t width, uint32_t height)
    : file(nullptr), width(width), height(height), fileFormat(ImageFormat::FileFormat::NONE) {
    AE_PROFILE_FUNCTION()
...
void OpenGLTexture2D::setData(void* data, uint32_t size) {
    ...
    if (imageData != nullptr) *imageData = *static_cast<uint8_t*>(data);

    GLenum dataFormat;
    switch (pixelFormat) {
        case ImageFormat::RGB: dataFormat = GL_RGB;
            break;
        case ImageFormat::RGBA: dataFormat = GL_RGBA;
            break;
        default:
