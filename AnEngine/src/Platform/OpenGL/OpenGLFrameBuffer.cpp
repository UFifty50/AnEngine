...
namespace Utils {
    ...
    static GLenum GLformatType(FrameBufferTexFormat format) {
        ...
        switch (format) {
            case FrameBufferTexFormat::RGBA8: return GL_UNSIGNED_BYTE;
            case FrameBufferTexFormat::RED_INTEGER: return GL_INT;
            default:
                AE_CORE_ASSERT(false, "Unknown or unsupported format!");
                return 0;
        }
    ...
    static uint32_t GLsizeOf(FrameBufferTexFormat format) {
        ...
        switch (format) {
            case FrameBufferTexFormat::RGBA8: return 4;
            case FrameBufferTexFormat::RED_INTEGER: return 1;
            default:
                AE_CORE_ASSERT(false, "Unknown or unsupported format!");
                return 0;
        }
...
OpenGLFrameBuffer::OpenGLFrameBuffer(const FrameBufferSpec& spec) : specification(spec) {
    ...
    for (const auto& attachmentSpec : specification.Attachments.Attachments) {
        if (attachmentSpec.texFormat.isDepth()) depthAttachmentSpec = attachmentSpec;
        else colourAttachmentSpecs.emplace_back(attachmentSpec);
    }
...
std::vector<int32_t> OpenGLFrameBuffer::readPixels(uint32_t attachmentIndex,
    ...
    AE_PROFILE_FUNCTION()
        ...
        AE_CORE_ASSERT(from.x >= 0 && from.x <= static_cast<int>(specification.Width), "from.x out of range!");
        AE_CORE_ASSERT(from.y >= 0 && from.y <= static_cast<int>(specification.Height), "from.y out of range!");
    // clang-format on
    ...
    glReadPixels(static_cast<GLsizei>(from.x), static_cast<GLsizei>(from.y),
...
void OpenGLFrameBuffer::reconstruct() {
    ...
    if (colourAttachmentSpecs.size() > 0) {
        ...
        for (uint32_t i = 0; i < colourAttachments.size(); i++) {
            ...
            switch (colourAttachmentSpecs[i].texFormat) {
                ...
                default: {
                    std::string colourFormat = colourAttachmentSpecs[i].texFormat.toString();
                    AE_CORE_ASSERT(false, "Unknown or unsupported colour texture format {}!",
                                   colourFormat);
                }
            }
        }
    ...
    if (depthAttachmentSpec.texFormat != FrameBufferTexFormat::None) {
        ...
        switch (depthAttachmentSpec.texFormat) {
            ...
            default: {
                std::string depthFormat = depthAttachmentSpec.texFormat.toString();
