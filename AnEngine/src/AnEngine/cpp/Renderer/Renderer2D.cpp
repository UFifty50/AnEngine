...
void Renderer2D::init() {
    ...
    for (uint32_t i = 0; i < MaterialBatch::maxIndices; i += 6) {
        rendererData.quadIndices[static_cast<size_t>(i) + 0] = offset + 0;
        rendererData.quadIndices[static_cast<size_t>(i) + 1] = offset + 1;
        rendererData.quadIndices[static_cast<size_t>(i) + 2] = offset + 2;

        rendererData.quadIndices[static_cast<size_t>(i) + 3] = offset + 2;
        rendererData.quadIndices[static_cast<size_t>(i) + 4] = offset + 3;
        rendererData.quadIndices[static_cast<size_t>(i) + 5] = offset + 0;
    ...
    rendererData.cameraUniformBuffer =
        UniformBuffer::Create(sizeof(Storage::CameraData), 0);
...
void Renderer2D::newMaterialBatch(const Material& material) {
...

    {
...

        {
            ...
            else { (*rendererData.materialBatchesHead)[material].push_back(batch); }
        }
...
void Renderer2D::shutdown() {
    ...
    //   rendererData.activeMaterial = AnEngine::UUID(nullptr);
...
void Renderer2D::endScene() { // draw batch
    ...
    rendererStats.usedMaterials = rendererData.materialBatchesHead->size();
...
void Renderer2D::flushActiveBatch() {
    ...
    // Bind textures
    for (uint32_t i = 0; i < batch.textureSlotIndex; i++) { batch.textureSlots[i]->bind(i); }
...
void Renderer2D::drawSprite(const glm::mat4& transform,
    ...
    if (rendererData.activeMaterial != sprite.Mat &&
        !rendererData.materialBatchesHead->contains(sprite.Mat))
        newMaterialBatch(sprite.Mat);

    rendererData.activeMaterial = sprite.Mat;

    if (const auto& tex = sprite.Mat.getTexture()) { drawQuad(transform, *tex, entityID); }
    else { drawQuad(transform, sprite.Mat.colour, entityID); }
}
...
void Renderer2D::drawQuad(const glm::mat4& transform, const glm::vec4& colour,
    ...
    if (batch.indexCount >= MaterialBatch::maxIndices) { newBatch(); }
...
void Renderer2D::drawQuad(const glm::mat4& transform, const Ref<Texture2D>& texture,
    ...
    if (batch.indexCount >= MaterialBatch::maxIndices ||
        batch.textureSlotIndex >= MaterialBatch::maxTextureSlots) { newBatch(); }
