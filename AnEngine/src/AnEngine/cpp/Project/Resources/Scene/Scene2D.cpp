...
void Scene2D::onUpdateEditor(TimeStep deltaTime, const Ref<EditorCamera>& camera) {
    ...
    for (auto entity : spriteGroup) {
        ...
        Renderer2D::drawSprite(transform, sprite, static_cast<uint32_t>(entity));
    }
...
void Scene2D::onUpdateRuntime(TimeStep deltaTime) {
    ...
    if (mainCamera) {
        ...
        for (auto entity : spriteGroup) {
            ...
            if (static_cast<uint32_t>(entity) > static_cast<unsigned>(std::numeric_limits<
                int32_t>::max())) {
                AE_CORE_ASSERT(false, "Too many entities in scene :("); // TODO: fixme
            }

            Renderer2D::drawSprite(transform, sprite, static_cast<int32_t>(entity));
        }
