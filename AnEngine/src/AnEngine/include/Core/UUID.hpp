...
struct UUID {
    ...
    UUID(const uint32_t id0, const uint32_t id1) {
        id[0] = id0;
        id[1] = id1;
    }
...
struct UUIDHasher {
    size_t operator()(const AnEngine::UUID& uuid) const {
        const size_t hash1 = std::hash<uint32_t>{}(uuid.id[0]);
        const size_t hash2 = std::hash<uint32_t>{}(uuid.id[1]);
