...
public:
    ...
    struct MaterialBatch {
        static constexpr uint32_t maxQuads = 100'000;
    ...
    struct MatList {
        ...
        ~MatList() { if (next != nullptr) delete next; }

        std::vector<MaterialBatch>& operator[](const Material& material) {
            if (this->material == material) return this->batches;
            if (next != nullptr) return (*next)[material];
            next = new MatList(material);
            return next->batches;
        }

        MaterialBatch& getBatch(const Material& material) {
            if (this->material == material) return this->batches.back();
            if (next != nullptr) return next->getBatch(material);
            else
        ...
        void clearUnused() {
            if (next != nullptr) {
                ...
                else { next->clearUnused(); }
            }
        }

        bool contains(const Material& material) {
            if (this->material == material) return true;
            if (next != nullptr) return next->contains(material);
            return false;
        }

        uint32_t size() {
            if (next != nullptr) return next->size() + batches.size();
            return batches.size();
        }
        ...
        void resetBatches() {
            ...
            if (next != nullptr) { next->resetBatches(); }
        }

        // iterator that returs [material, batches] pairs
        class MatListIterator {
            using value_type = std::pair<Material, std::vector<MaterialBatch>>;

        public:
            MatListIterator(MatList* ptr = nullptr) : ptr(ptr) {}

            operator bool() const { return ptr != nullptr; }
            bool operator==(const MatListIterator& other) const { return ptr == other.ptr; }
